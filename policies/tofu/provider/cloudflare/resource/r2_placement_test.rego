...
test_missing_location_is_denied if {
	...
	some m in msgs
	contains(m.msg, "roots/example/r2.tf: cloudflare_r2_bucket.state must set location explicitly")
}
...
test_every_explicit_argument_is_reported if {
	...
	every argument, _ in explicit_arguments {
		some m in msgs
		contains(m.msg, sprintf("must set %s explicitly", [argument]))
	}
}
...
test_missing_argument_names_its_reason if {
	...
	some m in msgs
	contains(m.msg, "must set jurisdiction explicitly: R2 otherwise uses the default jurisdiction")
}

test_unknown_location_hint_is_denied if {
	msgs := deny with input as testdata.bucket_with({"location": "eu-central-1"})
	some m in msgs
	contains(m.msg, "sets location = \"eu-central-1\", which is not one of apac, eeur, enam, oc, weur, wnam")
}

test_unknown_jurisdiction_is_denied if {
	msgs := deny with input as testdata.bucket_with({"jurisdiction": "pl"})
	some m in msgs
	contains(m.msg, "sets jurisdiction = \"pl\", which is not one of default, eu, fedramp")
}

test_unknown_storage_class_is_denied if {
	msgs := deny with input as testdata.bucket_with({"storage_class": "Glacier"})
	some m in msgs
	contains(m.msg, "sets storage_class = \"Glacier\", which is not one of InfrequentAccess, Standard")
}
...
test_several_buckets_are_reported_separately if {
	...
	count(msgs) == 2
	some m in msgs
	contains(m.msg, "cloudflare_r2_bucket.assets must set location")
	some n in msgs
	contains(n.msg, "cloudflare_r2_bucket.state must set jurisdiction")
}
