...
test_credentials_in_the_endpoint_url_are_denied if {
	...
	some m in msgs
	contains(m.msg, "must not carry credentials in the URL")
}
...
test_plain_endpoint_is_not_read_as_carrying_credentials if {
	...
	every m in msgs {
		not contains(m.msg, "must not carry credentials in the URL")
	}
}
...
test_interpolated_endpoint_is_denied if {
	...
	count(msgs) == 1
	some m in msgs
	contains(m.msg, "a backend block cannot interpolate variables")
}
