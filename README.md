## tact

[![Tofu support](https://img.shields.io/badge/OpenTofu-supported-green?logo=opentofu&logoColor=white)](https://github.com/opentofu/opentofu)
[![Tofu FAQ](https://img.shields.io/badge/Tofu-FAQ-blue)](https://opentofu.org/docs/)
[![Get tofu help](https://img.shields.io/badge/Tofu-help%20%26%20support-6f42c1)](https://github.com/kortega-com/tact/issues/new/choose)

### Support for tofu users

If you need help using the tofu actions in this repository, use these resources:

- **FAQ / docs:** [OpenTofu documentation](https://opentofu.org/docs/)
- **Report issues:** [Open a GitHub issue](https://github.com/kortega-com/tact/issues/new/choose)
- **Find common tofu issues:** [Search existing issues for `tofu`](https://github.com/kortega-com/tact/issues?q=is%3Aissue+tofu)
